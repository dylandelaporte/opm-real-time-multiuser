    ...
    socket.onmessage = function (data) {
        ...
        try {
            ...
            if (data.g) {
                if (data.g.mode === 0) {
                    if (currentMode === 1) {
                        console.log("currentMode reload");
                        location.reload();
                    }

                    controls.setup(socket, username, data.g.controls.local, data.g.controls.autoSetup);
                }
                else {
            ...
            if (data.m) {
                if (!mouses.list[data.id]) {
                    if (data.id === username) {
                        if (controls.isFirst()) {
                            controls.setFirst(data.m.left, data.m.top);
                        }
                    }
...
window.addEventListener("unload", function () {
    if (socket !== null) {
        socket.send(JSON.stringify({type: "disconnect", userId: getCookie(username_cookie_name)}));
    }
