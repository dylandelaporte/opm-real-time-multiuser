...
const coreSocketServer = io(3000, {pingInterval: 15000, pingTimeout: 10000});
const devicesSocketServer = io(5000);

const devices = require("./modules/devices-core.module");
...
devicesSocketServer.on("connection", client => {
    ...
    client.on("mouse", device => {
        devices.mouseUpdate(device.path, device.data);
    });

    client.on("keyboard", device => {
        devices.keyboardUpdate(device.path, device.data);
    });

    client.on("delete", device => {
        devices.deleteDevice(device.path);
    });
});
...
let packet = {
    ...
    send: function (client) {
        ...
        if (client) {
            ...
            coreSocketServer.emit("data", packet.data);
        }
...
coreSocketServer.on("connection", client => {
    ...
    client.on("refresh.device", () => {
        ...
        devicesSocketServer.emit("refresh");
    });
