...
The `Napi::Error` is a persistent reference (for more info see: [`Napi::ObjectReference`](doc/object_reference.md))
to a JavaScript error object. Use of this class depends on whether C++
exceptions are enabled at compile time.

If C++ exceptions are enabled (for more info see: [Setup](doc/setup.md)), then the
...
If C++ exceptions are disabled (for more info see: [Setup](doc/setup.md)), then the
...
to check for, get, and clear a pending JavaScript exception (for more info see: [Env](doc/env.md)).
